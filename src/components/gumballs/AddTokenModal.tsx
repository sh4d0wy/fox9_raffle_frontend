    ...
    <div className="mb-6">
        ...
        <div className="flex-1 grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="0"
              className="w-full h-12 px-4 rounded-lg border border-white/30 bg-black-1300 text-white font-medium font-inter focus:outline-none focus:border-primary-color transition"
            />
          </div>
          <div>
            <label className="text-sm text-white font-medium font-inter mb-2 block">
            ...
            <input
              ...
              min="1"
              className={clsx(
                "w-full h-12 px-4 rounded-lg border bg-black-1300 focus:border-primary-color text-white font-medium font-inter focus:outline-none transition",
                exceedsLimit 
      ...
      <div className="flex items-center gap-4 mt-3 ml-20 text-sm font-medium font-inter">
        <span className="text-primary-color">
          Total value: <span className="text-white">{totalValue.toFixed(7)} SOL</span>
        </span>
...
export const AddTokenModal = ({ isOpen, onClose, gumballId, remainingPrizes }: AddTokenModalProps) => {
  ...
  return (
    <Dialog
      ...
      onClose={handleClose}
    >
      <div className="fixed inset-0 z-10 w-screen h-full overflow-y-auto bg-black/70 [::webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <div className="flex min-h-full items-center justify-center p-4">
          <DialogPanel
            transition
            className="w-full max-w-[600px] rounded-2xl bg-black-1300 border-2 border-white/20 shadow-2xl duration-300 ease-out data-closed:transform-[scale(95%)] data-closed:opacity-0"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 pt-6 pb-4">
              <h4 className="text-2xl text-white font-bold font-inter">Add tokens</h4>
              <button
                onClick={handleClose}
              ...
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="white" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* Content */}
            <div className="px-6 pb-4 min-h-[30vh] overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-black-1300 [&::-webkit-scrollbar-thumb]:bg-white/30 [&::-webkit-scrollbar-thumb]:rounded-full">
              {/* Token Prize Rows */}
              ...
              {availableTokens.length > 0 && !hasReachedEntryLimit && (
                <div className="mt-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="w-16 h-16 rounded-xl bg-black-1300 shrink-0 border border-white/30 flex items-center justify-center">
                      {selectedToken ? (
                        <img 
                      ...
                      <Listbox value={selectedToken} onChange={setSelectedToken}>
                        {({ open }) => (
                          <div className="relative">
                            <ListboxButton
                              className={clsx(
                                "relative w-full h-12 px-4 rounded-lg border border-white/30 bg-black-1300 cursor-pointer text-left font-inter font-medium focus:outline-none focus:border-primary-color transition",
                            ...
                            <ListboxOptions
                              className={clsx(
                                "absolute z-100 w-full mt-1 bg-black-1300 border border-white/30 rounded-lg shadow-lg max-h-60 overflow-auto",
                            ...
                            >
                              {availableTokens.map((token) => (
                                <ListboxOption
                                  ...
                                  value={token}
                                  className={clsx(
                                    "flex items-center  gap-3 px-4 py-3 text-white cursor-pointer font-inter font-medium transition",
                                    "data-selected:bg-primary-color data-selected:text-black-1000",
                                    "data-focus:bg-primary-color/20 hover:bg-primary-color/20"
                                  )}
                                >
                                  <img 
                                    src={token.image} 
                      ...
                      <button
                        ...
                        className={clsx(
                          ...
                          actualRemainingPrizes < 1 
                            ...
                            : "text-primary-color hover:text-black-1000 cursor-pointer hover:bg-primary-color"
                        )}
                      >
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
