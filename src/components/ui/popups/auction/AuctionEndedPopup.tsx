...
interface AuctionEndedPopupProps {
  isOpen: boolean;
  shouldEnableAutoClose?: boolean;
}

export default function AuctionEndedPopup({ isOpen, shouldEnableAutoClose = true }: AuctionEndedPopupProps) {
  return (
    <PopupModal isOpen={isOpen} showCloseButton={false} shouldEnableAutoClose={shouldEnableAutoClose}>
      <div className="flex h-full flex-col  items-center justify-start gap-4">
        <div className="w-full flex flex-col items-center justify-center gap-4 pb-2">
          <h1 className="md:text-2xl text-xl text-center font-bold text-white">Auction Ended</h1>
        ...
        <h1 className="md:text-4xl text-3xl text-center font-bold text-primary-color">Waiting for Winner</h1>
        <Loader2 className="md:w-20 md:h-20 w-15 h-15 text-primary-color animate-spin" />
        <p className="text-sm text-white font-medium font-inter">The auction has ended winner will be announced soon...</p>
        </div>
      </div>
    </PopupModal>
  );
